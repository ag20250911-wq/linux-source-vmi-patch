 ...
 * pin_user_pages(). Remote mappings are excluded.
...
mm_remote_get_page(struct vm_fault *vmf)
{
	...
	int locked = 1;			/* mmap_lock is locked */
	...
	if (vmf->flags & FAULT_FLAG_KILLABLE) {
		if (down_read_killable_nested(&req_mm->mmap_lock, SINGLE_DEPTH_NESTING) == -EINTR) {
			pr_debug("%s: killed!\n", __func__);
			return ERR_PTR(-EINTR);
		}
	} else
		down_read_nested(&req_mm->mmap_lock, SINGLE_DEPTH_NESTING);

	/* first check VMA while holding remote mmap_lock */
	...
	nrpages = pin_user_pages_remote(req_mm, req_addr, 1,
					gup_flags, &req_page, NULL, plocked);

	if (locked)
		up_read(&req_mm->mmap_lock);
	...
	if (IS_ERR_VALUE(nrpages)) {
		pr_err("pin_user_pages_remote() failed: %d\n", (int)nrpages);
...
static int pidfd_mem_remap(struct remote_file_context *fctx, unsigned long address)
{
	...
	/* we have current->mm->mmap_lock in write mode, so no faults going on */
	down_write(&view->rmap_lock);
	rmap_interval_tree_remove(ctx, &view->rb_rmap);
	up_write(&view->rmap_lock);
